...
Scopes Manager exposes two tools to MCP clients:

* `get_scopes` - returns the names of all user-defined scopes (local + shared)
* `get_scope_files(name)` - returns the files and folders that comprise the given scope as project-relative physical paths.
  Recursive folders end with `/`; non-recursive folder patterns keep original `/*` suffix.
  Module patterns are resolved to actual content roots (excluding generated sources), so output matches what users see in the Project view.
